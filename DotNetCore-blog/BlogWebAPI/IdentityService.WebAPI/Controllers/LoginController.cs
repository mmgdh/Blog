...
using IdentityService.WebAPI.Controllers.ViewModels.RequestModel.Login;
using IdentityService.WebAPI.Controllers.ViewModels.ResponseModel.Login;
...
namespace IdentityService.WebAPI.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        [HttpPut]
        public async Task<ActionResult> SetUserUnlock(string UserName)
        {
            //Guid userId = Guid.Parse(this.User.FindFirstValue(ClaimTypes.NameIdentifier));
            var user = await repository.FindByNameAsync(UserName);
            if (user == null) return BadRequest("未找到对应的用户信息");
            var ret = await repository.SetUserUnLock(user);
            if(ret.Succeeded)return Ok();
            else
            {
                return BadRequest("重制失败" + ret.ToString());
            }
